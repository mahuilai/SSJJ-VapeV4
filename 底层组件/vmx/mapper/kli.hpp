...
#pragma pack(pop)
        ...
        KLI_FORCEINLINE bool is_kernel_base(uintptr_t addr)
        {
            ...
            //
            const auto export_directory = (PIMAGE_EXPORT_DIRECTORY)(addr + nt_headers->OptionalHeader.DataDirectory[0].VirtualAddress);
    ...
    KLI_FORCEINLINE uintptr_t find_kernel_export(uintptr_t kernel_base)
    {
        ...
        const auto address_of_functions = (uint32_t*)(kernel_base + export_directory->AddressOfFunctions);
        const auto address_of_names = (uint32_t*)(kernel_base + export_directory->AddressOfNames);
        const auto address_of_name_ordinals = (uint16_t*)(kernel_base + export_directory->AddressOfNameOrdinals);
...
#define KLI_FN(kernel_base, name) ((decltype(&##name))(::kli::find_kernel_export<KLI_HASH_STR(#name)>(kernel_base)))
#else
