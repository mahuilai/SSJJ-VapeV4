...
bool service::RegisterAndStart(const std::wstring& driver_path)
{
	...
	{
		printf("[service] RegCreateKeyW failed: 0x%08X\n", status);
		return false;
	}
	...
	if (status != ERROR_SUCCESS) 
	{
		printf("[service] RegSetKeyValueW(ImagePath) failed: 0x%08X\n", status);
	...
	if (status != ERROR_SUCCESS) {
		printf("[service] RegSetKeyValueW(Type) failed: 0x%08X\n", status);
		RegCloseKey(dservice);
		return false;
	}
	...
	if (!NT_SUCCESS(Status)) {
		printf("[service] NtLoadDriver failed: 0x%08X\n", (unsigned)Status);
		return false;
	}
