...
HANDLE intel_driver::Load()
{
	...
	// 以下三步是"痕迹清理"（把 provider 从驱动枚举/签名缓存/卸载记录里摘掉），
	// 非加载必需。旧版 kdmapper 的固定 AOB 特征码在 Win11 26200+ 上不匹配，
	// 若清理失败仅打警告继续，不影响 R/W 原语与手动映射功能。
	if (!intel_driver::ClearPiDDBCacheTable(result)) 
	{
		printf("[WARN] ClearPiDDBCacheTable failed (continue; iqvw64e.sys is MS-signed, low detection value)\n");
	}

	if (!intel_driver::ClearKernelHashBucketList(result)) 
	{
		printf("[WARN] ClearKernelHashBucketList failed (continue)\n");
	}

	if (!intel_driver::ClearMmUnloadedDrivers(result)) 
	{
		printf("[WARN] ClearMmUnloadedDrivers failed (continue)\n");
	}
