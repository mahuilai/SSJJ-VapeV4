...
namespace Vape.Feature.Legit
{
    public class SoftAim : MonoBehaviour
    {
        ...
        private static Vector3 GetBacktrackSoftAimBoneition(BacktrackRecord record)
        {
            if (Config.SoftAimBone == 0 || Config.SoftAimBone == 1)
            {
                return record.HeadPosition;
            }
            if (Config.SoftAimBone == 2 || Config.SoftAimBone == 3)
            {
            ...
            Vector3 position)
        {
            try
            {
                BacktrackAimState.SelectFromSoftAim(
                    index,
                    record,
                    entityId,
                    Config.HistoryAutoShoot);
        ...
        private static bool RefreshHeldBacktrackSelection(PlayerInfo target)
        {
            try
            {
                ...
                for (int i = 0; i < records.Count; i++)
                {
                    ...
                    {
                        Vector3 position = GetBacktrackSoftAimBoneition(record);
        ...
        private void UpdateTarget()
        {
            ...
            if (_isActive && _currentTarget != null && !_currentTarget.IsDead)
            {
                ...
                else if (Config.SoftAimVisCheck && !IsVisible(_currentTarget))
                {
                    _currentTarget = null;
                    _isActive = false;
                    ResetBacktrackSelection();
            ...
            foreach (var player in PlayerUpdate.EntityList)
            {
                ...
                try
                {
                    ...
                    {
                        BacktrackRecord record = records[i];
                        Vector3 targetPos = GetBacktrackSoftAimBoneition(record);
                        if (targetPos == Vector3.zero)
                        {
                            continue;
                        }

                        if (Config.SoftAimVisCheck && !IsVisible(player, targetPos))
                        {
                            continue;
                        }
        ...
        private void ProcessAiming()
        {
            ...
            Vector3 targetPosition = _selectedBacktrackRecord != null
                ...
                : GetTargetBonePosition(_currentTarget);
