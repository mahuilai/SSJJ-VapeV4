...
namespace Vape.Feature.Combat
{
    internal static class TraceUtility
    {
        ...
        {
            if (_mins == null) _mins = new float[3];
            if (_maxs == null) _maxs = new float[3];
            if (_traceStart == null) _traceStart = new float[3];
            if (_traceEnd == null) _traceEnd = new float[3];
        }
        ...
        {
            if (shooter == null || target == null || shooter.IsDead() || target.IsDead())
                return false;

            Contexts contexts = Contexts.sharedInstance;
            if (contexts == null || contexts.battleRoom == null ||
                contexts.battleRoom.pyEngine == null ||
                contexts.battleRoom.pyEngine.PyEngine == null)
                return false;

            if (shooter.move == null || shooter.move.PyPlayerMove == null ||
                shooter.move.PyPlayerMove.GetEntity() == null ||
                target.move == null || target.move.PyPlayerMove == null ||
            ...
            try
            {
                ...
                if (world == null || bulletTrace.EndPos == null || bulletTrace.EndPos.Length < 3)
                    return false;
                ...
                Vector3 bulletEnd = new Vector3(
                    bulletTrace.EndPos[0], bulletTrace.EndPos[1], bulletTrace.EndPos[2]);
                ...
                Vector3 worldEnd = new Vector3(trace.endPos[0], trace.endPos[1], trace.endPos[2]);
