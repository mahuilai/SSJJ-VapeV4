...
namespace Vape.Feature.Visuals
{
    public class EspMaster : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            bool drawTrail = BacktrackManager.Enabled && Config.HistoryTrail;
            int count = list.Count;
            for (int i = 0; i < count; i++)
            {
                var player = list[i];
                if (player == null || player.IsDead || player.Team == localTeam) continue;
                if (drawTrail) DrawBacktrack(player);
                DrawEnemy(player);
            }
        }
        ...
        private static void DrawBacktrack(PlayerInfo player)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    GUI.color = new Color(Vape.UI.Theme.EnemyBacktrack.r, Vape.UI.Theme.EnemyBacktrack.g, Vape.UI.Theme.EnemyBacktrack.b, fade * Vape.UI.Theme.EnemyBacktrack.a);
        ...
        private bool TryGetBoundingBox(PlayerInfo player, out Rect rect, out Color color)
        {
            ...
            if (SoftAim._currentTarget?._entity == player._entity)
