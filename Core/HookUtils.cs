...
namespace Vape.Core.Hook
{
    public static unsafe class HookUtils
    {
        ...
        public static void SetAddrFlagsToRWX(IntPtr ptr, int size)
        {
            ...
            //    string errorMsg = $"修改内存权限失败! 地址: 0x{ptr.ToInt64():X}, 大小: {size}, 错误码: {errorCode}";
            //    ////global::System.Console.WriteLine($"[严重错误] {errorMsg}");
            //    throw new Exception(errorMsg);
            //}
            //else
            //{
            //    ////global::System.Console.WriteLine($"[成功] 内存权限修改成功 地址: 0x{ptr.ToInt64():X}, 大小: {size}");
            //}
#else
        ...
        {
            #if Debug_Log
            global::System.Console.WriteLine($"[信息] Windows平台自动处理指令缓存 地址: 0x{(IntPtr.Size == 4 ? (uint)code : (ulong)code):x}");
            #endif
            if (code == null)
                return;
...
#if Debug_Log
            global::System.Console.WriteLine($"flush icache at 0x{(IntPtr.Size == 4 ? (uint)code : (ulong)code):x}, size:{size}");
#endif
        }
...
#if Debug_Log
            global::System.Console.WriteLine($"flush_icache delegate is {((flush_icache != null) ? "not " : "")}null");
#endif
        }
